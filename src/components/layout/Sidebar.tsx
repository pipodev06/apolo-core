...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-50 w-64 transform border-r border-gray-200 bg-white transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0",
      ...
      <div className="flex h-full flex-col">
        <div className="flex h-16 items-center gap-2 border-b border-gray-200 px-6">
          <div className="flex w-full items-center justify-center gap-2">
            ...
            <span className="text-lg font-bold text-gray-800">Sistema de Tickets</span>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-4">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={cn("mr-3 h-5 w-5", isActive ? "text-indigo-600" : "text-gray-800")}
                />
                {item.label}
              </Link>
            );
          ...
          {papeleraVisible && (
            ...
            >
              <Trash2 className={cn("mr-3 h-5 w-5", papeleraActiva ? "text-indigo-600" : "text-gray-800")} />
              Papelera
            </Link>
          )}
        </nav>
